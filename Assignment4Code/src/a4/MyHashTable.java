...
class MyHashTable<K,V> {
	...
	public  V  put(K key, V value) {
		...
		HashLinkedList<K,V> tmpLinkedList = this.buckets.get(hashCode);
		if (tmpLinkedList.getListNode(key)==null) {
			tmpLinkedList.add(key, value);
			this.entryCount++;
		}
		else {
			V replacedValue = tmpLinkedList.getListNode(key).getValue();
			tmpLinkedList.add(key,value);
			return replacedValue;
		}
		float tmpEntryCount = (float) this.entryCount;
		float tmpNumBuckets = (float) this.numBuckets;
		Float load = tmpEntryCount/tmpNumBuckets;
		if (load>this.MAX_LOAD_FACTOR) {
			this.rehash();
		}
		//  ADD YOUR CODE ABOVE HERE
		return null;
	...
	public V get(K key) {
		...
		HashLinkedList<K,V> currLinkedList = this.buckets.get(index);
		for (int i=0;i<currLinkedList.size();i++) {
			if (currLinkedList.getListNode(key)!=null) {
				V tmpValue = currLinkedList.getListNode(key).getValue();
				return tmpValue;
			}
		}
		//  ADD YOUR CODE ABOVE HERE
	...
	public V remove(K key) {
		...
		//  ADD YOUR CODE BELOW HERE
		HashLinkedList<K,V> currLinkedList = this.buckets.get(this.hashFunction(key));
		for (int i=0;i<currLinkedList.size();i++) {
			if (currLinkedList.getListNode(key)!=null) {
				this.entryCount--;
				return currLinkedList.remove(key).getValue();
			}
		}
		//  ADD  YOUR CODE ABOVE HERE
	...
	{
		//   ADD YOUR CODE BELOW HERE
		int newNumBuckets = this.getNumBuckets()*2;
		MyHashTable<K,V> newHashTable = new MyHashTable<K,V>(newNumBuckets);
		for (int i=0;i<this.getNumBuckets();i++) {
			HashLinkedList<K,V> currLinkedList = this.buckets.get(i);
			if (currLinkedList!=null) {
				for (int j=0;j<currLinkedList.size();j++) {
					HashNode<K,V> tmpNode = currLinkedList.getNode(j);
					newHashTable.put(tmpNode.getKey(), tmpNode.getValue());
				}
			}
		}
	...
	public ArrayList<K>  keys()
	{
		...
		for (int i=0;i<this.getNumBuckets();i++) {
			HashLinkedList<K,V> currLinkedList = this.buckets.get(i);
			if (currLinkedList.isEmpty()==true) {
				continue;
			}
			for (int j = 0;j<currLinkedList.size();j++) {
				if (currLinkedList.getNode(j)!=null) {
					listKeys.add(currLinkedList.getNode(j).getKey());
				}
			}
		}
		return listKeys;
	...
	public ArrayList <V> values()
	{
		...
		for (int i = 0;i<this.getNumBuckets();i++) {
			...
			for (int j = 0;j<currLinkedList.size();j++) {
				if (currLinkedList.getNode(j)!=null) {
					listValues.add(currLinkedList.getNode(j).getValue());
				}
			}
		}
		return listValues;
	...
	public class HashIterator implements  Iterator<HashNode<K,V> > {
		...
		public  HashIterator()
		{
			...
			for (int i=getNumBuckets()-1;i>=0;i--) {
				HashLinkedList<K,V> currLinkedList = buckets.get(i);
				if (currLinkedList.isEmpty()==true) {
					continue;
				}
				else {
					for (int j=currLinkedList.size()-1;j>=0;j--) {
						tmp.add(currLinkedList.getNode(j).getKey(), currLinkedList.getNode(j).getValue());
					}
				}
			}
			this.allEntries = tmp;
